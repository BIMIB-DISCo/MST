...
infer = function(x, y, ann, ...) {
	if (y == "PRIM") 
		m = tronco.prim(x, nboot = NBOOT, epos = EPOS, eneg = ENEG)
	if (y == "EDMONDS") 
		m = tronco.edmonds(x, nboot = NBOOT, epos = EPOS, eneg = ENEG)
	if (y == "CHOWLIU") 
		m = tronco.chowliu(x, nboot = NBOOT, epos = EPOS, eneg = ENEG)
	if (y == "GABOW") 
		m = tronco.gabow(x, nboot = NBOOT, epos = EPOS, eneg = ENEG)
	if (y == "CAPRESE") 
		m = tronco.caprese(x, epos = EPOS, eneg = ENEG)
	if (y == "CAPRI") 
		m = tronco.capri(x, nboot = NBOOT, epos = EPOS, eneg = ENEG)
...
data.merged = data
save(data.merged, file='data.merged.RData')
