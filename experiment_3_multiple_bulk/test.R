...
reconstruct.and.plot <- function(this.experiment, scite.tree, exp.code) {
    ...
    print(exp.code)
    dev.new()
    oncoprint(dataset, title = exp.code, file = paste0(exp.code, '_oncoprint.pdf'))
    dev.off()
...
## select a dataset
sample_sizes_single_cells = c(5, 7, 10, 20, 50)
...
epos = e_pos_multiple_biopses[[noise]]
eneg = e_neg_multiple_biopses[[noise]]
branching = 'random_20'
...
for (exp in 1:100) {
    for(noise in 1:5) {
        ...
        ## WTF?!? MA 20181209
        exp.code = paste0('/home/dex/project/MST/experiment_3_multiple_bulk/test/',
                          exp.code)
