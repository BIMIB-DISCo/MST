...
for (type in c('accuracy', 'hamming_distance', 'sensitivity', 'specificity')) {
    for (branching in c('low')) {
        ...
        pdf(paste('plot_reduced/mt', type, branching, '.pdf', sep='_'), height = 5, width = 5)
        multiplot(plotlist = plotlist)
        dev.off()

        pdf(paste('plot_reduced/mt_jitter', type, branching, '.pdf', sep='_'), height = 5, width = 5)
