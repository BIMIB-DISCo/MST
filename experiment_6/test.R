...
####linear.tree.onconem = recon.onconem$best$tree
    ...
    model.pmi.gabow =
        recon.gabow$model$gabow_no_reg_pmi$adj.matrix$adj.matrix.fit
    amat(net[[1]]) = model.pmi.gabow
    score.pmi.gabow =
        bnlearn::score(net[[1]], categoric.dataset, type = 'loglik')

    model.pmi.gabow.false =
        recon.gabow.false$model$gabow_no_reg_pmi$adj.matrix$adj.matrix.fit
    amat(net[[2]]) = model.pmi.gabow.false
    score.pmi.gabow.false =
        bnlearn::score(net[[2]], categoric.dataset, type = 'loglik')

    model.mi.gabow =
        recon.gabow$model$gabow_no_reg_mi$adj.matrix$adj.matrix.fit
    amat(net[[3]]) = model.mi.gabow
    score.mi.gabow =
        bnlearn::score(net[[3]], categoric.dataset, type = 'loglik')

    model.mi.gabow.false =
        recon.gabow.false$model$gabow_no_reg_mi$adj.matrix$adj.matrix.fit
    amat(net[[4]]) = model.mi.gabow.false
    score.mi.gabow.false =
        bnlearn::score(net[[4]], categoric.dataset, type = 'loglik')

    model.cpmi.gabow =
        recon.gabow$model$gabow_no_reg_cpmi$adj.matrix$adj.matrix.fit
    amat(net[[5]]) = model.cpmi.gabow
    score.cpmi.gabow =
        bnlearn::score(net[[5]], categoric.dataset, type = 'loglik')

    model.cpmi.gabow.false =
        recon.gabow.false$model$gabow_no_reg_cpmi$adj.matrix$adj.matrix.fit
    amat(net[[6]]) = model.cpmi.gabow.false
    score.cpmi.gabow.false =
        bnlearn::score(net[[6]], categoric.dataset, type = 'loglik')

    model.entropy.gabow =
        recon.gabow$model$gabow_no_reg_entropy$adj.matrix$adj.matrix.fit
    amat(net[[7]]) = model.entropy.gabow
    score.entropy.gabow =
        bnlearn::score(net[[7]], categoric.dataset, type = 'loglik')

    model.entropy.gabow.false =
        recon.gabow.false$model$gabow_no_reg_entropy$adj.matrix$adj.matrix.fit
    amat(net[[8]]) = model.entropy.gabow.false
    score.entropy.gabow.false =
        bnlearn::score(net[[8]], categoric.dataset, type = 'loglik')

    model.cpmi.edmonds =
        recon.edmonds$model$edmonds_no_reg_cpmi$adj.matrix$adj.matrix.fit
    amat(net[[9]]) = model.cpmi.edmonds
    score.edmonds.cpmi =
        bnlearn::score(net[[9]], categoric.dataset, type = 'loglik')

    model.pmi.edmonds =
        recon.edmonds$model$edmonds_no_reg_pmi$adj.matrix$adj.matrix.fit
    amat(net[[10]]) = model.pmi.edmonds
    score.edmonds.pmi =
        bnlearn::score(net[[10]], categoric.dataset, type = 'loglik')

    model.entropy.edmonds =
        recon.edmonds$model$edmonds_no_reg_entropy$adj.matrix$adj.matrix.fit
    amat(net[[11]]) = model.entropy.edmonds
    score.edmonds.entropy =
        bnlearn::score(net[[11]], categoric.dataset, type = 'loglik')

    model.bic.capri =
        recon.capri$model$capri_bic$adj.matrix$adj.matrix.fit
    amat(net[[12]]) = model.bic.capri
    score.capri.bic =
        bnlearn::score(net[[12]], categoric.dataset, type = 'loglik')

    model.caprese =
        recon.caprese$model$caprese$adj.matrix$adj.matrix.fit
    amat(net[[13]]) = model.caprese
    score.caprese =
        bnlearn::score(net[[13]], categoric.dataset, type = 'loglik')

    model.prim =
        recon.prim$model$prim_no_reg$adj.matrix$adj.matrix.fit
    amat(net[[14]]) = model.prim
    score.prim =
        bnlearn::score(net[[14]], categoric.dataset, type = 'loglik')

    model.chowliu =
        recon.chowliu$model$chow_liu_loglik$adj.matrix$adj.matrix.fit
    amat(net[[15]]) = model.chowliu
    score.chowliu =
        bnlearn::score(net[[15]], categoric.dataset, type = 'loglik')

    model.scite =
        scite.tree
    amat(net[[20]]) = model.scite
    score.scite =
        bnlearn::score(net[[20]], categoric.dataset, type = 'loglik')
    ...
    if (TRUE
        ...
        print(exp.code)
        dev.new()
        oncoprint(recon.gabow,
                  title = exp.code,
                  file = paste0(exp.code, '_oncoprint.pdf'))
        dev.off()
...
branching = 'random_forest'
sample.type = 'single'
##dev.new()

dataset = dataset.random.single.cells.forest.nodes
...
for (exp in 1:100) {
    ...
    filename = paste0('scite_output/datasets/',
                      branching,  '/',
                      sample.type, '/',
                      sample.scite, '_', exp, '_', noise, '_ml0')
