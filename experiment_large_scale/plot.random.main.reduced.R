...
for (type in c('sensitivity', 'specificity', 'elasped_time')) {
    for (branching in c('random_20')) {
        ...
        for (sample in samples) {
            p = dotplotter(results.values, sample, c('chowliu_loglik'), 
                'large_20',
                type,
                paste('SAMPLE SIZE = ', sample),
                noise = c(1),
        ...
        pdf(paste('plot_reduced/chowliu', type, branching, '.pdf', sep='_'), height = 10, width = 9)
        multiplot(plotlist = plotlist)
        dev.off()
        ...
        for (sample in samples) {
            p = dotplotter(results.values, sample, c(
                'edmonds_pmi.no.reg',
                'chowliu_loglik'), 
                'large_20',
                type,
