...
run.reconstructions <- function( dataset, epos, eneg, debug = FALSE, seed = NA, pass.error.rates = TRUE) {
    ...
    capri = list(bic.adj=adj.matrix.capri.bic, score = score)
    results[["capri"]] = capri
    ...
    caprese = list(adj.caprese=adj.matrix.caprese, score = score)
    ...
    edmonds = list(pmi.no.reg.adj = adj.matrix.edmonds.pmi.no.reg, score = score)
    ...
    gabow = list(pmi.no.reg.adj = adj.matrix.gabow.pmi.no.reg, score = score)
    results[["gabow"]] = gabow
    ...
    chowliu = list(loglik.adj=adj.matrix.chowliu.loglik, score = score)
    ...
    prim = list(no.reg.adj=adj.matrix.prim.no.reg, score = score)
    results[["prim"]] = prim
