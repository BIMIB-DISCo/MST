...
run.reconstructions <- function( dataset, epos, eneg, debug = FALSE, seed = NA, pass.error.rates = TRUE) {
    ...
    capri = list(adj=adj.matrix.capri.bic, score = score, model = res)
    results[["capri"]] = capri
    ...
    caprese = list(adj=adj.matrix.caprese, score = score, model = res)
    ...
    edmonds = list(adj = adj.matrix.edmonds.pmi.no.reg, score = score, model = res)
    ...
    prim = list(adj=adj.matrix.prim.no.reg, score = score, model = res)
    results[["prim"]] = prim
