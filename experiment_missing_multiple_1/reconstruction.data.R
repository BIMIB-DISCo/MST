...
run.reconstructions <- function( dataset, true.tree, epos, eneg, debug = FALSE, seed = NA, pass.error.rates = TRUE) {
    ...
    capri = list(adj=adj.matrix.capri.bic, score = score, result = result)
    results[["capri"]] = capri
    ...
    caprese = list(adj=adj.matrix.caprese, score = score, result = result)
    ...
    edmonds = list(adj = adj.matrix.edmonds.pmi.no.reg, score = score, result = result)
    ...
    gabow = list(pmi.no.reg.adj = adj.matrix.gabow.pmi.no.reg, score = score, result = result)
    results[["gabow"]] = gabow
    ...
    chowliu = list(loglik.adj=adj.matrix.chowliu.loglik, score = score, result = result)
    ...
    prim = list(adj=adj.matrix.prim.no.reg, score = score, result = result)
    results[["prim"]] = prim
...
experiment.missing.data = expand.input(dataset.missing.impute, seed = seed, cores = cores)
