...
for(sample in 1:nrow(experiment.missing.data)) {
    for (exp in 1:ncol(experiment.missing.data)) {
        ...
        for (imputated in 1:length(exec)) {
            ...
            experiment$reconstruction$prim$score =
                bnlearn::score(net[[3]], categoric.dataset, type = 'loglik')
            if (is.na(this.find.score$prim$score)
                || experiment$reconstruction$prim$score > this.find.score$prim$score) {
...
load('RData/experiments.single.cells.medium.scite.stats.RData')
