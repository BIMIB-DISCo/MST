...
colnames(dataset.missing.data) =
    paste("Experiment", 1:length(dataset_imputed))
...
for(i in 1:length(dataset_imputed)) {
    ...
    row.has.na <- apply(curr_imputation, 1, function(x){any(is.na(x))})
    if (sum(row.has.na) > 0) {
        curr_imputation = curr_imputation[!row.has.na, ]
    }
    exp = list()
