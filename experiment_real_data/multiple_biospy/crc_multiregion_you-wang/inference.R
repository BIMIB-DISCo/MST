...
tofix = consolidate.data(data, print = T)
data$genotypes =
    data$genotypes[, which(colnames(data$genotypes) != "G37")]
data$genotypes =
    data$genotypes[, which(colnames(data$genotypes) != "G38")]
data$annotations =
    data$annotations[which(rownames(data$annotations) != "G37"), ]
data$annotations =
    data$annotations[which(rownames(data$annotations) != "G38"), ]
data$annotations["G39", "event"] = "merged"
data$annotations["G39", "type"] = "indistinguishable"
data$types =
    data$types[which(rownames(data$types) != "NA"), , drop = FALSE]
...
sapply("CAPRI", FUN = infer, x = data, ann = "hypo", scale.nodes = 0.2)
