...
plot_graph <- function(model, title, file) {
    model =
        change.color(model, type = 'variant', new.color = 'lightblue3')
    oncoprint(model,
...
for (sample in 1:nrow(experiment.missing.data)) {
    for (exp in 1:ncol(experiment.missing.data)) {
        ...
        for (noise in 1:length(exec)) {
            ...
            ## capri
            if (is.na(find.score$capri$score)
                || experiment$reconstruction$capri$score > find.score$capri$score) {
                find.score$capri$score = experiment$reconstruction$capri$score
                find.score$capri$id = exp
                title = paste('\nCAPRI score:', round(find.score$capri$score, 3),
                              ' id: ', find.score$capri$id)
            ...
            ## caprese
            if (is.na(find.score$caprese$score)
                || experiment$reconstruction$caprese$score > find.score$caprese$score) {
                find.score$caprese$score = experiment$reconstruction$caprese$score
                find.score$caprese$id = exp
                title = paste('\nCAPRESE score:', round(find.score$caprese$score, 3),
                              ' id: ', find.score$caprese$id)
            ...
            ## prim
            if (is.na(find.score$prim$score)
                || experiment$reconstruction$prim$score > find.score$prim$score) {
                find.score$prim$score = experiment$reconstruction$prim$score
                find.score$prim$id = exp
                title = paste('\nPRIM score:', round(find.score$prim$score, 3),
                              ' id: ', find.score$prim$id)
            ...
            ## chowliu
            if (is.na(find.score$chowliu$score)
                || experiment$reconstruction$chowliu$score > find.score$chowliu$score) {
                find.score$chowliu$score = experiment$reconstruction$chowliu$score
                find.score$chowliu$id = exp
                title = paste('\nCHOW LIU score:', round(find.score$chowliu$score, 3),
                              ' id: ', find.score$chowliu$id)
            ...
            ## edmonds
            if (is.na(find.score$edmonds$score)
                || experiment$reconstruction$edmonds$score > find.score$edmonds$score) {
                find.score$edmonds$score = experiment$reconstruction$edmonds$score
                find.score$edmonds$id = exp
                title = paste('\nEDMONDS score:', round(find.score$edmonds$score, 3),
                              ' id: ', find.score$edmonds$id)
            ...
            ## gabow
            if (is.na(find.score$gabow$score)
                || experiment$reconstruction$gabow$score > find.score$gabow$score) {
                find.score$gabow$score = experiment$reconstruction$gabow$score
                find.score$gabow$id = exp
                title = paste('\nGABOW score:', round(find.score$gabow$score, 3),
                              ' id: ', find.score$gabow$id)
