...
run.reconstructions <- function(dataset,
    ...
    capri = list(bic.adj = adj.matrix.capri.bic,
                 score = score,
                 model = res)
    results[["capri"]] = capri
    ...
    caprese = list(adj.caprese = adj.matrix.caprese,
                   score = score,
                   model = res)
    ...
    edmonds = list(pmi.no.reg.adj = adj.matrix.edmonds.pmi.no.reg,
                   score = score,
                   model = res)
    ...
    ## Performs the reconstructions with Prim no_reg, loglik, aic and
    ## bic
    ...
    prim = list(no.reg.adj = adj.matrix.prim.no.reg,
                score = score,
                model = res)
    results[["prim"]] = prim
...
for(i in 1:nrow(dataset.missing.data)) {
    for (j in 1:ncol(dataset.missing.data)) {
        cat((((i - 1) * ncol(dataset.missing.data)) + j) , '/',
            nrow(dataset.missing.data) * ncol(dataset.missing.data), '\n')
        single.experiment = dataset.missing.data[[i, j]]
        results = lapply(single.experiment,
...
### end of file -- reconstruction.data.R
