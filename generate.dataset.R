...
generate.dataset.single.cells.convergent <- function (type,
    ...
    if (type == "medium") {
        if (!is.null(true_tree)) {
            for (i in samples_num) {
                for (j in 1:length(e_pos)) {
                    random_dataset = sample.random.single.cells.convergent(i,e_pos[j],e_neg[j],ncol(true_tree),min_significance,max_significance,samples_significance,true_tree)
                    results[[as.character(i)]][[as.character(j)]][["dataset"]] = random_dataset$sampled_dataset
                    results[[as.character(i)]][[as.character(j)]][["true_tree"]] = random_dataset$random_tree$structure
                    results[[as.character(i)]][[as.character(j)]][["probabilities"]] = random_dataset$random_tree$probabilities
                    results[[as.character(i)]][[as.character(j)]][["root_prob"]] = random_dataset$random_tree$root_prob
...
# generate a random tree and the probabilities of the associated single cell samples
generate.random.single.cell.dataset.convergent <- function ( nodes, min_significance = 0.6, max_significance = 0.90, true_tree = NULL ) {
    ...
    # (NOTE: this is NOT extended for any confluence, but specific for the topology in the example)
    curr_conv.prob = root.prob * probabilities[1,2] * probabilities[1,3] * (1-probabilities[1,4]) * probabilities[2,5] * probabilities[3,5] * (1-probabilities[5,7])
    random_dataset = rbind(random_dataset,c(1,1,1,0,1,0,0,0,curr_conv.prob))
    curr_conv.prob = root.prob * probabilities[1,2] * probabilities[1,3] * (1-probabilities[1,4]) * probabilities[2,5] * probabilities[3,5] * probabilities[5,7]
    random_dataset = rbind(random_dataset,c(1,1,1,0,1,0,1,0,curr_conv.prob))
    curr_conv.prob = root.prob * probabilities[1,3] * probabilities[1,4] * (1-probabilities[1,2]) * probabilities[3,6] * probabilities[4,6] * (1-probabilities[6,8])
    random_dataset = rbind(random_dataset,c(1,0,1,1,0,1,0,0,curr_conv.prob))
    curr_conv.prob = root.prob * probabilities[1,3] * probabilities[1,4] * (1-probabilities[1,2]) * probabilities[3,6] * probabilities[4,6] * probabilities[6,8]
