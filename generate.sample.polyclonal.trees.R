##############################################################################
###
### Generate Sample Polyclonal Tree
###
##############################################################################
...
sample.single.cells.polyclonal.medium <- function(samples_num,
    ...
    ## Randomly sample the nodes probabilities if not provided
    if (length(nodes_probabilities) == 0 || is.na(nodes_probabilities)) {
...
sample.random.single.cells <- function(samples_num,
                                       e_pos,
                                       e_neg,
                                       nodes,
                                       min_significance = 0.60,
                                       max_significance = 0.9,
                                       sample_significance = 0.005,
                                       true_tree = NULL) {
    ...
    while (is.null(random_tree) || 
           ...
           max(random_tree$dataset_samples[,"Probs"]) >= (1 - sample_significance)) {
               ...
               random_tree = generate.random.single.cell.dataset(nodes,
                                                                 min_significance,
                                                                 max_significance,
                                                                 true_tree)
           }
...
sample.random.single.cells.significant.tree <- function (nodes,
                                                         min_significance = 0.6,
                                                         max_significance = 0.90,
                                                         sample_significance = 0.005,
                                                         true_tree = NULL) {
    
    ## Generate a random tree with minimum significance per
    ## samples
    random_tree = NULL
    while(is.null(random_tree) || 
          min(random_tree$dataset_samples[,"Probs"]) <= sample_significance || 
          max(random_tree$dataset_samples[,"Probs"]) >= (1 - sample_significance)) {
              ...
              ## Keep generating random trees until I get a valid
              ## one
              random_tree = generate.random.single.cell.dataset(nodes,
                                                                min_significance,
                                                                max_significance,
...
### Sample one observation of multiple biospes from a given single
### cell model
...
sample.multiple.biospes.from.single.cells <- function(clones_num,
    ...
    ## If a mutation is observed at least in 1 sample, it is observed
    ## (OR of the samples)
...
sample.multiple.biopses.polyclonal.low <- function(samples_num,
    ...
    sampled_dataset = sapply(res.matrix[, 1],
                             function(x) {
...
sample.random.multiple.biopses <- function(samples_num,
    ...
    random_tree = sample.random.single.cells.significant.tree(nodes,
                                                              min_significance,
                                                              max_significance,
                                                              samples_significance,
                                                              true_tree)
    
    ## As an heuristics, one bulk sample is the mix of nodes/2 single
    ## cells samples
    clones_per_sample = round(nodes / 2)
    
    res.matrix = matrix(NA, ncol = nodes, nrow = (samples_num - wild_type_samples))
    res = sapply(res.matrix[, 1],
                 function(x) {
    ...
    ## Save the results
    res = list(random_tree = random_tree,
...
sample.random.single.cells.forest <- function(samples_num,
                                              e_pos,
                                              e_neg,
                                              nodes,
                                              min_significance = 0.60,
    ...
    for (i in 1:num.trees) {
        tree = sample.random.single.cells(samples_num,
                                          0,
                                          0,
                                          num.nodes[[i]],
                                          min_significance,
        ...
        left = rbind(final.values, matrix(0,
                                          ncol = ncol(final.values),
                                          nrow = nrow(values)))
        right = rbind(matrix(0,
                             ncol = ncol(values),
                             nrow = nrow(final.values)),
                      values)
    ...
    random_tree = list(structure = final.true.tree,
                       dataset_samples = final.dataset.samples)
    res = list(random_tree = random_tree,
               sampled_dataset = final.dataset)
    return(res)
}
...
sample.random.single.cells.forest.fixed.tree <- function(samples_num,
                                                         e_pos,
                                                         e_neg,
                                                         min_significance = 0.60,
                                                         max_significance = 0.9,
...
sample.random.single.cells.convergent <- function(samples_num,
                                                  e_pos,
                                                  e_neg,
                                                  nodes,
                                                  min_significance = 0.60,
    ...
    while (is.null(random_tree) || 
           ...
           max(random_tree$dataset_samples[, "Probs"]) >= (1 - sample_significance)) {
              ...
              ## Keep generating random trees until I get a valid one
              random_tree = generate.random.single.cell.dataset.convergent(nodes,
                                                                           min_significance,
                                                                           max_significance,
                                                                           true_tree)
          }
...
### end of file -- generate.sample.polyclonal.trees.R
