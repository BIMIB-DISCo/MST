    ...
    for (sample in 1:nrow(dataset)) {
        ...
        for (experiment in 1:ncol(dataset)) {
            ...
            for (noise in 1:length(execution)) {
                ...
                write.table(genotype, file = paste0('sifit_input/', filename), quote = FALSE, sep = ' ', col.names = FALSE, row.names = FALSE)
                script = paste0(script, 'printf "EXEC: ', count, '/800 - sample: ', sample, ' experiment: ', experiment, ' noise lev: ', noise, ' \\n" \n')
                script = paste0(script, 'java -jar ../SiFit.jar -df 1 -ipMat ', filename, ' -n ', nmuts, ' -m ', nsample, ' -r 1 -iter ', steps,' -fp ', epos_level, ' -fn ', eneg_level, ' \n')
                count = count + 1
            }
        }
