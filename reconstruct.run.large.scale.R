...
run.reconstructions <- function(dataset,
    ...
    results.edmonds.entropy.no.reg = getStats(true_tree, adj.matrix.edmonds.entropy.no.reg[["edmonds_no_reg_entropy"]], res)
    
    adj.matrix.edmonds.pmi.no.reg = as.adj.matrix(res, model = "edmonds_no_reg_pmi")
    results.edmonds.pmi.no.reg = getStats(true_tree, adj.matrix.edmonds.pmi.no.reg[["edmonds_no_reg_pmi"]], res)

    adj.matrix.edmonds.cpmi.no.reg = as.adj.matrix(res, model = "edmonds_no_reg_cpmi")
    results.edmonds.cpmi.no.reg = getStats(true_tree, adj.matrix.edmonds.cpmi.no.reg[["edmonds_no_reg_cpmi"]], res)
    ...
    results.chowliu.loglik = getStats(true_tree, adj.matrix.chowliu.loglik[["chow_liu_loglik"]], res)
...
getStats <- function(true_matrix,
    ...
    ## register elasped time
    time = res$execution.time[[3]]
...
reconstruct.and.score <- function(dataset,
                                  epos,
                                  eneg,
                                  debug = FALSE,
                                  seed = NA,
    ...
    res = tronco.capri(data,
                       regularization = c("loglik", "aic", "bic"),
                       epos = epos,
                       eneg = eneg,
                       silent = silent,
                       boot.seed = seed)
    ...
    adj.matrix.capri.bic = as.adj.matrix(res, model = "capri_bic")
    results.capri.bic = getStats(true_tree, adj.matrix.capri.bic[["capri_bic"]])
    capri = list(loglik.adj = adj.matrix.capri.loglik,
                 loglik.res = results.capri.loglik,
                 aic.adj = adj.matrix.capri.aic,
                 aic.res = results.capri.aic,
                 bic.adj = adj.matrix.capri.bic,
                 bic.res = results.capri.bic)
    results[["capri"]] = capri
...
### end of file -- reconstruct.run.large.scale.R
