...
run.reconstructions <- function(dataset, true_tree, epos, eneg) {
    ...
    ## Performs the reconstructions with Edmonds no_reg, loglik, aic
    ## and bic
    res = NULL
    res = tronco.mst.edmonds(data,
    ...
    edmonds = list(entropy.no.reg.adj = adj.matrix.edmonds.entropy.no.reg,
                   entropy.no.reg.res = results.edmonds.entropy.no.reg,
                   pmi.no.reg.adj = adj.matrix.edmonds.pmi.no.reg,
                   pmi.no.reg.res = results.edmonds.pmi.no.reg,
                   cpmi.no.reg.adj = adj.matrix.edmonds.cpmi.no.reg,
                   cpmi.no.reg.res = results.edmonds.cpmi.no.reg)
    results[["edmonds"]] = edmonds

    ## Performs the reconstructions with Gabow
    res = NULL
    res = tronco.mst.gabow(data,
                           regularization = c("no_reg"),
    ...
    gabow = list(entropy.no.reg.adj = adj.matrix.gabow.entropy.no.reg,
                 entropy.no.reg.res = results.gabow.entropy.no.reg,
                 pmi.no.reg.adj = adj.matrix.gabow.pmi.no.reg,
                 pmi.no.reg.res = results.gabow.pmi.no.reg,
                 cpmi.no.reg.adj = adj.matrix.gabow.cpmi.no.reg,
                 cpmi.no.reg.res = results.gabow.cpmi.no.reg,
                 mi.no.reg.adj = adj.matrix.gabow.mi.no.reg,
                 mi.no.reg.res = results.gabow.mi.no.reg)
    results[["gabow"]] = gabow
    ...
    gabow = list(no.raising.entropy.no.reg.adj = adj.matrix.gabow.no.raising.entropy.no.reg,
                 no.raising.entropy.no.reg.res = results.gabow.no.raising.entropy.no.reg,
                 no.raising.pmi.no.reg.adj = adj.matrix.gabow.no.raising.pmi.no.reg,
                 no.raising.pmi.no.reg.res = results.gabow.no.raising.pmi.no.reg,
                 no.raising.cpmi.no.reg.adj = adj.matrix.gabow.no.raising.cpmi.no.reg,
                 no.raising.cpmi.no.reg.res = results.gabow.no.raising.cpmi.no.reg,
                 no.raising.mi.no.reg.adj = adj.matrix.gabow.no.raising.mi.no.reg,
                 no.raising.mi.no.reg.res = results.gabow.no.raising.mi.no.reg)
    results[["gabow_no_rising"]] = gabow
    
    ## Performs the reconstructions with Chow Liu loglik, aic and bic
    res = NULL
    res = tronco.mst.chowliu(data,
...
### end of file -- reconstruction.run.slmh.R
