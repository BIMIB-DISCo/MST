...
performance.plot <- function(dataset,
    ...
    if (! branching %in% branching.types) {
...
compare.performance.plot <- function(dataset,
    ...
    for (type in names(dataset)) {
        ...
        print(wireframe(mean~x*y,
                        ...
                        scales = list(arrows = FALSE,
                                      y = list(at = 1:length(sample),
                                               labels = as.character(sample))),
        ...
        pdf(file = paste0(branching, '/',
                          sample.type, '/',
                          type, '/',
                          'gabow_scite_compare', '.pdf'),
            width = 8.5,
            height = 6.5)

        zlim = c(min(results.compare.gabow[, 'mean']) - 0.05,
                 max(results.compare.gabow[, 'mean']) + 0.05)
...
compare.performance.plot.2d <- function(dataset, sample.type, branching) {
...
##            dev.off()
            ...
            pdf(file = paste0(branching, '/',
                ...
                width = 8.5,
                height = 6.5)
            ylim = c(min(res[, 'mean']) - max(res[, 'sd']) - 0.05,
                     max(res[, 'mean']) + max(res[, 'sd']) + 0.05)
            ...
            pdf(file = paste0(branching, '/',
                              sample.type, '/',
                              type, '/',
                              s, '_', 'gabow_no_raising_scite', '.pdf'),
                width = 8.5,
                height = 6.5)
            ylim = c(min(res[, 'mean']) - max(res[, 'sd']) - 0.05,
                     max(res[, 'mean']) + max(res[, 'sd']) + 0.05)
...
##            dev.off()
            ...
            pdf(file = paste0(branching, '/',
                              sample.type, '/',
                              type, '/',
                              s, '_', 'gabow_scite_median', '.pdf'),
                width = 8.5,
                height = 6.5)
            ylim = c(min(res[, 'median']) - 0.05,
                     max(res[, 'median']) + 0.05)
            mycolors = brewer.pal(length(ordered.regs.c7), 'Paired')[1:length(ordered.regs.c7)]
            print(xyplot(median~x,
                         grid = TRUE,
            ...
            pdf(file = paste0(branching, '/',
                              sample.type, '/',
                              type, '/',
                              s, '_', 'gabow_no_rais_scite_median', '.pdf'),
                width = 8.5,
                height = 6.5)
            ylim = c(min(res[, 'median']) - 0.05, max(res[, 'median']) + 0.05)
            mycolors = brewer.pal(length(ordered.regs.c8), 'Paired')[1:length(ordered.regs.c8)]
...
add.alpha <- function(col, alpha = 1) {
    ...
    r = apply(sapply(col, col2rgb) / 255,
              2,
              function(x) rgb(x[1], x[2], x[3], alpha = alpha))
    return(as.vector(r))
}

### end of file -- statistics.plot.R
