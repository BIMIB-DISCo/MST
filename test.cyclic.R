...
reconstruct.and.plot <- function(this.experiment,
                                 scite.tree,
                                 exp.code,
                                 epos,
                                 eneg) {
    ...
    score.scite = bnlearn::score(net[[4]], categoric.dataset, type = 'loglik')
...
for (exp in 1:100) {
    ...
    filename = paste0('../MST/scite_output/',
                      branching, '/',
                      sample.type, '/',
    ...
    reconstruct.and.plot(this.experiment, scite.tree, exp.code, epos, eneg)
}
