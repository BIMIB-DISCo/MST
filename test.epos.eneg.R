...
p1 = ggplot(density.value.s, aes(x = values)) +
    ...
    geom_text(aes(label = paste('nboot =', nboot),
                  x = Inf, y = hmax),
              hjust = 0) +
    geom_text(aes(label = paste('epos =', epos),
                  x = Inf,
                  y = hmax - 0.5),
              hjust = 0) + 
    geom_text(aes(label = paste('eneg =', eneg),
                  x = Inf,
                  y = hmax - 1),
              hjust = 0) +
    geom_text(aes(label = paste('tp =', tp),
                  x = Inf,
                  y = hmax - 1.5),
              hjust = 0) +
    geom_text(aes(label = paste('pr =', pr),
                  x = Inf,
                  y = hmax - 2),
              hjust = 0) +
    geom_text(aes(label = paste('hg =', hg),
                  x = Inf,
                  y = hmax - 2.5),
              hjust = 0) +
    geom_text(aes(label = paste0('p(',from , ') =', mean(p.from)),
                  x = Inf,
                  y = hmax - 3),
              hjust = 0) +
    geom_text(aes(label = paste0('p(',to , ') =', mean(p.to)),
                  x = Inf,
                  y = hmax - 3.5),
              hjust = 0) + 
    geom_text(aes(label = paste0('p(',from, ',', to , ') =', mean(p.joint.from.to)),
                  x = Inf,
                  y = hmax - 4),
              hjust = 0)
    
p2 = ggplot(density.raising.value.s, aes(x = values)) + 
    geom_histogram(aes(y = ..density.., group = ind, colour = ind),
                   position = "identity",
                   size = 0, fill = NA) + 
    geom_density(aes(group = ind, colour = ind), alpha = .5) +
